...
func (s *Server) PromocionDelete(c echo.Context) error {
	promocionId, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
	...
	if err != nil {
		log.Error().Err(err).Int64("id", promocionId).Msg("Failed to delete promocion")
		return c.NoContent(http.StatusInternalServerError)
	}
...
func (s *Server) PromocionDetails(c echo.Context) error {
	...
	if err != nil {
		log.Error().Err(err).Interface("id", promocionId).Msg("Failed to read promocion")
		return c.NoContent(http.StatusInternalServerError)
	}
